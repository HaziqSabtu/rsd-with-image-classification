...
def train():
    ...
    # Download Dataset
    data_dir = r'C:\Users\kakik\Desktop\yolov5-classifier\GTSRB'
    train_dir = r'C:\Users\kakik\Desktop\yolov5-classifier\GTSRB\train'
    test_dir = r'C:\Users\kakik\Desktop\yolov5-classifier\GTSRB\test'
    ...
    testform = T.Compose(trainform.transforms)

    # Dataloaders
    trainset = torchvision.datasets.ImageFolder(root=train_dir, transform=trainform)
...
def classify2(model, size=128, file='../datasets/mnist/test/3/30.png', plot=False):
    # YOLOv5 classification model inference
    import cv2
    import numpy as np
    import torch.nn.functional as F
    ...
    p = F.softmax(results, dim=1)  # probabilities
    i = p.argmax()  # max index
    # print(f'{00} prediction: {i} ({p[0, i]:.2f})')
